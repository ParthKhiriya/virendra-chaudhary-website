      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col items-center text-center md:text-left md:items-start">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
          ...
          <div className="lg:col-span-5 w-full flex justify-center lg:justify-end mt-12 lg:mt-0" ref={imageRef}>
            <div className="relative w-full max-w-md aspect-[4/5] rounded-3xl overflow-hidden shadow-2xl group cursor-pointer">
              <img 
          ...
          {pointers.map((pointer, idx) => {
            ...
            return (
              <a 
                key={idx}
                href="#services"
                className="group relative p-5 md:p-8 rounded-2xl md:rounded-3xl bg-gray-50 border border-gray-200 hover:border-primary/50 transition-all duration-500 hover:-translate-y-2 hover:bg-white hover:shadow-lg overflow-hidden cursor-pointer flex flex-row items-center md:items-start md:flex-col gap-4 md:gap-0"
              >
                {/* Hover Glow Effect */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 blur-[50px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                
                <div className="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl bg-white border border-gray-100 shadow-sm flex items-center justify-center md:mb-6 group-hover:scale-110 transition-transform duration-500 group-hover:border-primary/30">
                  <Icon className="w-5 h-5 md:w-6 md:h-6 text-gray-900 group-hover:text-primary transition-colors duration-500" />
                </div>
                
                <h3 className="text-lg md:text-2xl font-bold text-gray-900 md:mb-4 tracking-wide group-hover:text-primary transition-colors duration-500 flex-1">
                  {pointer.title}
                </h3>
                ...
                {/* Minimalist Arrow */}
                <div className="absolute bottom-5 right-5 md:bottom-8 md:right-8 opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500">
