          ...
          <div className="lg:col-span-5 w-full flex justify-center lg:justify-end mt-12 lg:mt-0" ref={imageRef}>
            <div tabIndex={0} className="relative w-full max-w-md aspect-[4/5] rounded-3xl overflow-hidden shadow-2xl group cursor-pointer">
              <img 
                ...
                tabIndex={0} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 group-active:scale-105 group-focus:scale-105"
              />
              <div className="absolute inset-0 border border-black/5 rounded-3xl pointer-events-none" />
              {/* Subtle hover overlay */}
              <div tabIndex={0} className="absolute inset-0 bg-primary/0 group-hover:bg-primary/10 group-active:bg-primary/10 group-focus:bg-primary/10 transition-colors duration-500 pointer-events-none" />
            </div>
          </div>
        </div>
          ...
          {pointers.map((pointer, idx) => {
            ...
            return (
              <a 
                key={idx}
                href="#services"
                tabIndex={0} className="group relative p-5 md:p-8 rounded-2xl md:rounded-3xl bg-gray-50 border border-gray-200 hover:border-primary/50 active:border-primary/50 focus:border-primary/50 transition-all duration-500 hover:-translate-y-2 active:-translate-y-2 focus:-translate-y-2 hover:bg-white active:bg-white focus:bg-white hover:shadow-lg active:shadow-lg focus:shadow-lg overflow-hidden cursor-pointer flex flex-row items-center md:items-start md:flex-col gap-4 md:gap-0"
              >
                {/* Hover Glow Effect */}
                <div tabIndex={0} className="absolute top-0 right-0 w-32 h-32 bg-primary/20 blur-[50px] rounded-full opacity-0 group-hover:opacity-100 group-active:opacity-100 group-focus:opacity-100 transition-opacity duration-700 pointer-events-none" />
                
                <div tabIndex={0} className="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl bg-white border border-gray-100 shadow-sm flex items-center justify-center md:mb-6 group-hover:scale-110 group-active:scale-110 group-focus:scale-110 transition-transform duration-500 group-hover:border-primary/30 group-active:border-primary/30 group-focus:border-primary/30">
                  <Icon tabIndex={0} className="w-5 h-5 md:w-6 md:h-6 text-gray-900 group-hover:text-primary group-active:text-primary group-focus:text-primary transition-colors duration-500" />
                </div>
                
                <h3 tabIndex={0} className="text-lg md:text-2xl font-bold text-gray-900 md:mb-4 tracking-wide group-hover:text-primary group-active:text-primary group-focus:text-primary transition-colors duration-500 flex-1">
                  {pointer.title}
                </h3>
                ...
                {/* Minimalist Arrow */}
                <div tabIndex={0} className="absolute bottom-5 right-5 md:bottom-8 md:right-8 opacity-0 -translate-x-4 group-hover:opacity-100 group-active:opacity-100 group-focus:opacity-100 group-hover:translate-x-0 group-active:translate-x-0 group-focus:translate-x-0 transition-all duration-500">
